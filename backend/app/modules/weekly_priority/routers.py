...
@router.get("/weekly-plans", response_model=List[schemas.WeeklyPriorityPlanResponse])
def read_plans(
    headsup_id: int,
...
@router.post("/weekly-plans", response_model=schemas.WeeklyPriorityPlanResponse, status_code=status.HTTP_201_CREATED)
def create_plan(
...
@router.get("/weekly-plans/{id}", response_model=schemas.WeeklyPriorityPlanResponse)
def read_plan(
    id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(deps.get_current_user)
...
@router.put("/weekly-plans/{id}", response_model=schemas.WeeklyPriorityPlanResponse)
def update_plan(
    id: int,
